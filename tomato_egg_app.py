# app.py（完整：主餐 + 料理 + 飲料 + 採買交通(點地圖選分店) + 甜點(5選2) + 長條圖/圓餅圖(含比例) + 記錄結果(用報到姓名)）
...
def save_result(name: str, payload: dict):
    ...
    if os.path.exists(RESULT_PATH):
        df_row.to_csv(RESULT_PATH, mode="a", header=False, index=False, encoding="utf-8-sig")
    else:
        df_row.to_csv(RESULT_PATH, index=False, encoding="utf-8-sig")
...
# 甜點（第二階段）
st.session_state.setdefault("dessert_pool", None)  # DataFrame
st.session_state.setdefault("dessert_selected", [])  # list
...
with c1:
    if st.button("🎲 抽 3 項食材（主餐）", use_container_width=True):
        ...
        st.session_state.dessert_pool = None
        st.session_state.dessert_selected = []
...
if st.session_state.origin["lat"] is None or st.session_state.origin["lng"] is None:
    ...
    if not st.session_state.search:
        ...
        st.caption("提示：請點橘色分店 marker 附近；若點空白處不會改變選擇。")
...
total_stage1 = food_sum + cook_sum + drink_cf + transport_cf

st.subheader("✅ 第一階段碳足跡加總（主餐＋料理＋飲料＋採買交通）")
st.markdown(
    f"""
...
# 16) 圖表（長條圖 + 圓餅圖含比例 %）
# =========================
st.subheader("📊 第一階段圖表（即時更新）")

chart_stage1 = pd.DataFrame(
    [
...
pie_base = chart_stage1[chart_stage1["kgCO2e"] > 0].copy()
pie = (
    alt.Chart(pie_base)
    .transform_joinaggregate(total="sum(kgCO2e)")
    .transform_calculate(pct="datum.kgCO2e / datum.total")
    .mark_arc()
    .encode(
        ...
        tooltip=[
            ...
            alt.Tooltip("kgCO2e:Q", title="kgCO₂e", format=".3f"),
            alt.Tooltip("pct:Q", title="比例", format=".1%")
        ],
    )
...
pie_text = (
    ...
    .encode(text=alt.Text("pct:Q", format=".0%"), theta=alt.Theta("kgCO2e:Q"), color=alt.value("black"))
)
...
# =========================
# 17) 甜點（完成：已確認採買分店 才出現）→ 隨機 5 選 2
# =========================
dessert_cf = 0.0

if st.session_state.stores:
    st.subheader("🍰 第二階段：今日甜點（隨機 5 選 2）")

    if len(df_dessert) == 0:
        st.warning("Excel 找不到 code=3 的甜點資料，因此甜點碳足跡視為 0。")
        st.session_state.dessert_pool = None
        st.session_state.dessert_selected = []
    else:
        ...
        dessert_options = {
            r["product_name"]: float(r["cf_kgco2e"])
            for _, r in st.session_state.dessert_pool.iterrows()
        }

        selected = st.multiselect(
            "請選 2 種甜點（最多選 2）",
            options=list(dessert_options.keys()),
            default=st.session_state.dessert_selected,
            max_selections=2,
            key="dessert_select_ui",
        )
        st.session_state.dessert_selected = selected
        dessert_cf = sum(dessert_options[n] for n in selected)

        st.info(f"甜點碳足跡合計：**{dessert_cf:.3f} kgCO₂e**")
else:
    st.caption("（提示：請先在『採買交通』確認 1 家分店，甜點才會出現。）")


# =========================
# 18) 最終加總 + 最終圖表（含甜點）
# =========================
total_final = total_stage1 + dessert_cf

st.subheader("✅ 最終碳足跡（含甜點）")
st.markdown(
    f"""
- 第一階段總計：`{total_stage1:.3f}` kgCO₂e  
- 甜點：`{dessert_cf:.3f}` kgCO₂e  
- ✅ **最終總計**：**`{total_final:.3f}` kgCO₂e**
"""
)

st.subheader("📊 最終圖表（含甜點；比例 %）")
chart_final = pd.DataFrame(
    [
        {"項目": "Food", "kgCO2e": food_sum},
        {"項目": "Cooking", "kgCO2e": cook_sum},
        {"項目": "Drink", "kgCO2e": drink_cf},
        {"項目": "Transport", "kgCO2e": transport_cf},
        {"項目": "Dessert", "kgCO2e": dessert_cf},
    ]
)

bar2 = (
    alt.Chart(chart_final)
    .mark_bar()
    .encode(
        y=alt.Y("項目:N", sort="-x", title=""),
        x=alt.X("kgCO2e:Q", title="kgCO₂e"),
...
pie_base2 = chart_final[chart_final["kgCO2e"] > 0].copy()
pie2 = (
    alt.Chart(pie_base2)
    .transform_joinaggregate(total="sum(kgCO2e)")
    .transform_calculate(pct="datum.kgCO2e / datum.total")
...
# 19) 儲存結果（用報到姓名，不匿名）
# =========================
st.subheader("💾 紀錄結果（用報到姓名）")

vid = st.session_state.visitor_id.strip()
student_name = VALID_IDS.get(vid, {}).get("name", vid if vid else "訪客")

st.write(f"本次紀錄者：**{student_name}**")

can_save = (len(st.session_state.stores) > 0)  # 至少要確認分店（完成第一階段決策）

if st.button("✅ 儲存本次結果到 results.csv", use_container_width=True, disabled=not can_save):
    payload = {
        "food": round(food_sum, 6),
        "cooking": round(cook_sum, 6),
        "drink": round(drink_cf, 6),
        "transport": round(transport_cf, 6),
        "dessert": round(dessert_cf, 6),
        "total_stage1": round(total_stage1, 6),
        "total_final": round(total_final, 6),
        "transport_mode": st.session_state.get("transport_mode", ""),
        "round_trip": bool(st.session_state.get("round_trip", True)),
        "store_name": st.session_state.stores[0]["name"] if st.session_state.stores else "",
        "store_dist_km_oneway": st.session_state.stores[0].get("dist_km", None) if st.session_state.stores else None,
        "dessert_selected": "、".join(st.session_state.dessert_selected) if st.session_state.dessert_selected else "",
    }
    save_result(student_name, payload)
    st.success("已儲存完成 ✅（results.csv）")

st.caption("若按鈕是灰色：請先在『採買交通』確認 1 家分店，完成決策後才能儲存。")
